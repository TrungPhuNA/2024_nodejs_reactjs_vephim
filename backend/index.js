...
app.post("/registration", (req, res) => {
    ...
    db.query(
        ...
        (err, data) => {
            if (err) {
                return res.status(500).json({message: "Có lỗi, Xin vui lòng thử lại!"});
            }

            return res
                .status(200)
                .json({message: "Đăng ký tài hoản thành công"});
        }
    );
});
...
app.post("/login", (req, res) => {
    ...
    db.query(sql, [email, password], (err, data) => {
        ...
        if (data.length === 0) {
            return res.status(404).json({message: "Lỗi, không tồn tại user!"});
        }
